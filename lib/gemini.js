// lib/gemini.js
// V1.2 — Core analysis logic for Gemini with Google Search grounding
//
...
//
// V1.2: REMOVED all limits - shows everything API returns
...
const CONFIG = {
  ...
  // NO MAX_FANOUTS - show everything from API
};
...
/* =========================
   FANOUT PROCESSING - NO LIMITS
========================= */

/**
 * Process Gemini's web search queries into fanouts.
 * NO LIMITS - returns everything from the API.
...
function processFanouts(webSearchQueries, originalQuery) {
  ...
  for (const q of webSearchQueries) {
    ...
    result.push(q);
    seen.add(normalized);
    // NO LIMIT - add all valid queries
  }
...
export async function analyzeQuery(query, apiKey) {
  ...
  // Process fanouts (use Gemini's actual search queries - NO LIMITS)
