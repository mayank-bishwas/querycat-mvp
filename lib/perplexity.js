// lib/perplexity.js
// V1.0 — Core analysis logic for Perplexity Search API
//
...
const CONFIG = {
  ...
  MAX_RESULTS: 5, // Perplexity API param
};
...
// Noise words to filter from fanouts
...
// Action words = query-style fanout (good)
...
// Location words = entity-style fanout (limit these)
...
function processFanouts(rawFanouts, originalQuery) {
  // Perplexity doesn't give us a rewritten query, so we use the original
  const queryNormalized = originalQuery.toLowerCase().trim();
  
  const scored = rawFanouts
    .filter(f => f && f.toLowerCase().trim() !== queryNormalized)
    .map(f => {
  ...
  // Start with the original query as the first fanout
...
export async function analyzeQuery(query, apiKey) {
  ...
  // Perplexity always searches, but might return no results
  if (results.length === 0) {
    return {
      llm: "perplexity",
      query: cleanedQuery,
  ...
  // Generate fanout queries from title + URL
  const rawFanouts = [];
  ...
  const fanoutQueries = processFanouts(rawFanouts, cleanedQuery);
